...
export type DomainVariant = "data" | "ai" | "simulation" | "platform";
...
export type DomainVisualContent = {
  title: Localized;
  icon: UiIconName;
  image: string;
  imageAlt: Localized;
...
const DOMAIN_VISUAL_CONTENT: Record<DomainVariant, DomainVisualContent> = {
  data: {
    title: {
...
export function getDomainVisualContent(variant: DomainVariant) {
  return DOMAIN_VISUAL_CONTENT[variant];
}

export function getDomainVisualRows(variant: DomainVariant, lang: SiteLang) {
  return DOMAIN_VISUAL_CONTENT[variant].rows.map((row) => row[lang]);
}

export function getDomainVisualMetric(variant: DomainVariant, lang: SiteLang) {
  return DOMAIN_VISUAL_CONTENT[variant].metric[lang];
}
